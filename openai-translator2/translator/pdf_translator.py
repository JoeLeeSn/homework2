...
class PDFTranslator:
    ...
    def translate_pdf(self,
                    ...
                    translation_role: str = '小说',
                    pages: Optional[int] = None):
        LOG.info(f"Translating {input_file} from {source_language} to {target_language} with {translation_role} role.")
        self.book = self.pdf_parser.parse_pdf(input_file, pages)

        for page_idx, page in enumerate(self.book.pages):
            for content_idx, content in enumerate(page.contents):
                # Translate content.original
                translation, status = self.translate_chain.run(content, source_language, target_language,translation_role)
